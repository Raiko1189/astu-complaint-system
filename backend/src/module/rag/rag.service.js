...
import { summarizeResponse, generateGreetingResponse } from "../../utils/summarise.js";
...
export const ragService = {
    async query(userQuery) {
        try {
            ...
            const lowerQuery = userQuery.toLowerCase().trim().replace(/[?!.]$/, "");

            if (greetings.includes(lowerQuery)) {
                const greetingResult = await generateGreetingResponse(userQuery);
                if (greetingResult.success) {
                    return {
                        success: true,
                        data: greetingResult.finalResponse
                    };
                }
            }
